...
function BurnIn({ active, onFinish }: { active: boolean; onFinish: Finish }) {
  ...
  const press = (cell: number) => { if (!active) return; if (phase === "idle") { started.current = Date.now(); show([Math.floor(Math.random() * 9)]); return; } if (phase !== "play") return; if (sequence[input] !== cell) { const score = Math.max(0, (sequence.length - 1) ** 2 * 10); setPhase("idle"); onFinish({ score, durationMs: Math.max(1000, Date.now() - started.current), label: `LEVEL ${Math.max(1, sequence.length - 1)}` }); return; } if (input + 1 === sequence.length) show([...sequence, Math.floor(Math.random() * (sequence.length >= 5 ? 16 : 9))]); else setInput((value) => value + 1); };
...
function LcdRun({ active, onFinish }: { active: boolean; onFinish: Finish }) {
  ...
  useEffect(() => { if (!running || !active) return; let id = 0; const tick = () => { const current = state.current; const moved = current.obstacles.map((item) => ({ ...item, row: item.row + 1 })).filter((item) => item.row <= 5); if (moved.some((item) => item.row === 5 && item.lane === current.lane)) { setRunning(false); onFinish({ score: current.score, durationMs: Math.max(1000, Date.now() - current.start), label: "CRASHED" }); return; } const nextScore = current.score + 1; const density = nextScore > 30 ? 2 : 1; const incoming = Math.random() < .72 ? Array.from({ length: density }, (_, index) => ({ lane: (Math.floor(Math.random() * 3) + index) % 3, row: 0 })) : []; current.obstacles = [...moved, ...incoming]; current.score = nextScore; setObstacles(current.obstacles); setScore(nextScore); id = window.setTimeout(tick, Math.max(180, 520 - nextScore * 7)); }; id = window.setTimeout(tick, 520); return () => clearTimeout(id); }, [active, onFinish, running]);
...
function SignalLock({ active, onFinish }: { active: boolean; onFinish: Finish }) {
  const [tuner, setTuner] = useState(50); const [target, setTarget] = useState(48); const [score, setScore] = useState(0); const [meter, setMeter] = useState(100); const running = useRef(false); const start = useRef(0); const hold = useRef(0); const values = useRef({ tuner: 50, target: 48, score: 0, meter: 100 });
  useEffect(() => { if (!running.current || !active) return; let frame = 0; let previous = performance.now(); const step = (now: number) => { const dt = Math.min(120, now - previous); previous = now; const current = values.current; const width = Math.max(6, 18 - current.score / 350); current.target = Math.max(width / 2, Math.min(100 - width / 2, current.target + Math.sin(now / 700) * dt * (.012 + current.score / 90000))); const locked = Math.abs(current.tuner - current.target) <= width / 2; if (locked) { hold.current += dt; current.score += (hold.current > 3000 ? 2 : 1) * dt / 100; current.meter = Math.min(100, current.meter + dt / 140); } else { hold.current = 0; current.meter -= dt / 65; } values.current = current; setTarget(current.target); setScore(Math.floor(current.score)); setMeter(current.meter); if (current.meter <= 0) { running.current = false; onFinish({ score: Math.floor(current.score), durationMs: Math.max(1000, Date.now() - start.current), label: "SIGNAL LOST" }); return; } frame = requestAnimationFrame(step); }; frame = requestAnimationFrame(step); return () => cancelAnimationFrame(frame); }, [active, onFinish]);
  useEffect(() => { if (!active) running.current = false; }, [active]);
  const move = (event: React.PointerEvent<HTMLDivElement>) => { const rect = event.currentTarget.getBoundingClientRect(); const next = Math.max(0, Math.min(100, (event.clientX - rect.left) / rect.width * 100)); if (!running.current) { running.current = true; start.current = Date.now(); hold.current = 0; values.current = { tuner: next, target: 48, score: 0, meter: 100 }; setScore(0); setMeter(100); } values.current.tuner = next; setTuner(next); };
  return <div className="beat-board signal-board"><div className="score-stack"><span>LOCK</span><strong>{score}</strong><span>POWER <b>{Math.max(0, Math.round(meter))}%</b></span></div><div className="timing-rail signal-rail" onPointerDown={(event) => { event.currentTarget.setPointerCapture(event.pointerId); move(event); }} onPointerMove={(event) => event.currentTarget.hasPointerCapture(event.pointerId) && move(event)} role="slider" tabIndex={0} aria-label="Drag to tune the signal" aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.round(tuner)}><div className="target-zone" style={{ top: `${target}%` }} /><div className="marker" style={{ top: `${tuner}%` }} /></div><div className="board-action">DRAG TO LOCK</div></div>;
}
