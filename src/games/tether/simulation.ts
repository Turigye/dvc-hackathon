...
function extend(state: TetherState) {
  while (state.anchors.length < LOOKAHEAD) {
    ...
    const step = state.anchors.length;
    const difficulty = Math.min(1, step / 26);
    const rise = 1.5 + difficulty * 0.9 + random(state) * 0.5;
    const spread = 1.4 + difficulty * 1.5;
    const dx = (random(state) * 2 - 1) * spread;
    const x = Math.max(-2.6, Math.min(2.6, last.x + dx));
    const roll = random(state);
    const kind: Anchor["kind"] = step < 6 ? "solid" : roll < 0.16 ? "decay" : roll < 0.34 ? "drift" : "solid";
    state.anchors.push({
      id: last.id + 1,
      x,
      y: last.y + Math.min(rise, APEX * 0.82),
...
export function step(previous: TetherState, dtMs: number, input: TetherInput): TetherState {
  ...
  for (const anchor of state.anchors) {
    ...
    if (distance <= CAPTURE_RADIUS + ORBIT_RADIUS && (!best || distance < best.distance)) {
...
export function gapIsReachable(from: Anchor, to: Anchor) {
  ...
  const maxRise = APEX + ORBIT_RADIUS + CAPTURE_RADIUS;
  const maxRange = (LAUNCH_SPEED * LAUNCH_SPEED) / GRAVITY + ORBIT_RADIUS + CAPTURE_RADIUS;
  return dy <= maxRise && dx <= maxRange;
}
